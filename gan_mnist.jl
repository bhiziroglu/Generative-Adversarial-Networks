...
function D_loss(Dnet,sizeZ,x,o,Gnet,atype)
  ...
  G_real = x
  D_real = logp(discriminator(Dnet,G_real,atype))
  D_fake = logp(1-discriminator(Dnet,G_fake,atype))
  return -sum(D_real + D_fake)/size(D_real,2)
end
...
function G_loss(Gnet,Dnet,sizeZ,atype,o)
  ...
  G_logit = logp(D_fake)
  return -sum(G_logit)/size(D_fake,2)
end
